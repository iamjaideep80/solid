...
#include "SOP_Solid_Extract_Sim.h"
...
{
	table->addOperator(
			new OP_Operator("solid_extract_sim", "Solid Extract Sim", SOP_Solid_Extract_Sim::myConstructor,
							SOP_Solid_Extract_Sim::myTemplateList, 1, // Min required sources
							1,	// Maximum sources
							0));
}
...
PRM_Template SOP_Solid_Extract_Sim::myTemplateList[] =
...
int * SOP_Solid_Extract_Sim::myOffsets = 0;
OP_Node *
SOP_Solid_Extract_Sim::myConstructor(OP_Network *net, const char *name, OP_Operator *op)
{
	return new SOP_Solid_Extract_Sim(net, name, op);
}
SOP_Solid_Extract_Sim::SOP_Solid_Extract_Sim(OP_Network *net, const char *name, OP_Operator *op) :
...
SOP_Solid_Extract_Sim::~SOP_Solid_Extract_Sim()
{
}
void SOP_Solid_Extract_Sim::initSystem()
{

}
OP_ERROR SOP_Solid_Extract_Sim::cookMySop(OP_Context &context)
{
...
const char *
SOP_Solid_Extract_Sim::inputLabel(unsigned inum) const
{
	switch (inum)
	{
	case 0:
